...
<body>
    ...
    <div class="container">
        ...
        <div class="parent-panel">
            ...
            <div class="header">
                <div>
                    <h1>Halo, Orang Tua {{ $parent->name }}! 👋</h1>
            ...
            <div class="stats-grid">
                <div class="glass-card stat-card stat-card-tunggakan">
                    ...
                    <div class="stat-value" style="color: var(--orange);" id="parentTunggakan">Rp {{ number_format($tunggakan, 0, ',', '.') }}</div>
                    @if($tunggakan > 0)
                        @php
                           $tunggakanInvoice = collect($invoices)->firstWhere('status', 'Tunggakan');
                        @endphp
                        <button class="btn btn-orange" style="margin-top: 1rem; width: 100%;" onclick="payInvoice({{ $tunggakanInvoice->id }})">Bayar Sekarang</button>
                    @endif
                </div>
                <div class="glass-card stat-card stat-card-lunas">
                    <div style="color: #666;">Status Lunas</div>
                    <div class="stat-value" style="color: var(--success);" id="parentLunas">Rp {{ number_format($lunas, 0, ',', '.') }}</div>
                </div>
                <div class="glass-card stat-card stat-card-menunggu">
                    <div style="color: #666;">Menunggu</div>
                    <div class="stat-value" style="color: var(--warning);" id="parentMenunggu">Rp {{ number_format($menunggu, 0, ',', '.') }}</div>
                </div>
            </div>
                ...
                <table id="invoiceTable">
                    ...
                    <tbody>
                        ...
                        <tr id="row-{{ $inv->id }}">
                            ...
                            <td>
                                <span class="badge badge-{{ strtolower($inv->status) }} status-badge">{{ $inv->status }}</span>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="glass-card">
                ...
                <div style="display: flex; gap: 1rem;">
                    ...
                    <div style="background: white; border-radius: 8px; padding: 0.5rem 1rem; display: flex; align-items: center; gap: 0.5rem; border: 1px solid var(--border-color);">
                        <img src="{{ $pm->logo_url }}" alt="{{ $pm->name }}" style="height: 20px;">
                ...
                <div id="activityFeed">
                    ...
                    <div class="activity-item">
                        ...
                        <div>
                            ...
                            <div style="color: #666; font-size: 0.9rem;">Rp {{ number_format($act->amount, 0, ',', '.') }} ({{ $act->description }}) - {{ $act->status }}</div>
                        </div>
                    </div>
                    @endforeach
                </div>
